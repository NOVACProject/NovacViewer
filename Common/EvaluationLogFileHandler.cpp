		...
		while(fgets(szLine, 8192, f)){
			...
			if(NULL != strstr(szLine, scanInfoStop)){
				if(foundDate && foundTime){
					...
					array.SetAtGrow(nScans, new CDateTime());
				}
			...
			if(inScanInfoSection){
				...
				if(pt = strstr(szLine, "starttime=")){
					if(3 == sscanf(pt+10, "%d:%d:%d", &tmpInt[0], &tmpInt[1], &tmpInt[2])){
						...
						foundTime = true;
					}else if(3 == sscanf(pt+10, "%d.%d.%d", &tmpInt[0], &tmpInt[1], &tmpInt[2])){
						scanStartTime.hour   = tmpInt[0];
						scanStartTime.minute = tmpInt[1];
						scanStartTime.second = tmpInt[2];
						foundTime = true;
...
void CEvaluationLogFileHandler::ParseFluxInformation(CWindFieldAndPlumeHeight &windField, double &flux, FILE *f){
	char szLine[8192];
	char *pt = NULL;
...
RETURN_CODE CEvaluationLogFileHandler::WriteEvaluationLog(const CString fileName){
	...
	for(int scanIndex = 0; scanIndex < this->m_scanNum; ++scanIndex){
		...
		CWindFieldAndPlumeHeight	&wind	= this->m_windField[scanIndex];
...
/** Takes the filename of an evaluation log and extracts the 
		Serial-number of the spectrometer, the date the scan was performed
		and the start-time of the scan from the filename. */
bool FileHandler::CEvaluationLogFileHandler::GetInfoFromFileName(const CString fileName, CDateTime &start, CString &serial, int &channel){
	CString name, sDate, sTime, resToken;
	int iDate, iTime;
	int curPos = 0;
