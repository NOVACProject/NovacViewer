...
void CWindFieldRecord::InsertWindField(const CWindFieldAndPlumeHeight &wind){

	// Add the new wind-field to the end
	m_windField.SetAt(m_windFieldNum++, CWindFieldAndPlumeHeight(wind));
...
RETURN_CODE CWindFieldRecord::InterpolateWindField(const CDateTime desiredTime, CWindFieldAndPlumeHeight &desiredWindField){
	...
	//	the desired one.
	CWindFieldAndPlumeHeight closestBefore, closestAfter;
	...
	for(int i = 0; i < m_windFieldNum; ++i){
		CWindFieldAndPlumeHeight &currentRecord = m_windField.GetAt(i); // <-- the current record in the database
