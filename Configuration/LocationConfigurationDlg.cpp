...
BEGIN_MESSAGE_MAP(CLocationConfigurationDlg, CPropertyPage)
	...
	ON_CBN_SELCHANGE(IDC_COMBO_ELECTRONICS,				OnChangeElectronics)
END_MESSAGE_MAP()
...
BOOL CLocationConfigurationDlg::OnInitDialog()
{
	...
	// The electronics-box combo-box
#ifdef _DEBUG
	m_comboElectronics.ResetContent();
	m_comboElectronics.AddString("Beck");
	m_comboElectronics.AddString("Axis");
#else
	// for the release-versions, hide the electronics combo-box for now
	m_comboElectronics.ShowWindow(SW_HIDE);
	SetDlgItemText(IDC_LABEL_ELBOX, "");
#endif
...
void CLocationConfigurationDlg::OnChangeScanner(){
	if(UpdateData(TRUE)){ // <-- first save the data in the dialog
		...
		// Update the electronics-box type
...
void CLocationConfigurationDlg::OnChangeVolcano(){
    ...
    // 2019-02-25: Removed data here. Read through config-file, was ftp://SERVER/wind/wind_%s.txt
	m_configuration->windSourceSettings.windFieldFile.Format("", g_volcanoes.m_simpleName[curSel]);
}
