...
/** Clears this storage of all data... */
void CEvaluatedDataStorage::Clear(){
	this->m_windData.RemoveAll();
	
	for(unsigned int scannerIndex = 0; scannerIndex < MAX_NUMBER_OF_SCANNING_INSTRUMENTS; ++scannerIndex){
		m_fluxIndex[scannerIndex] = 0;
	}
}
...
int CEvaluatedDataStorage::AddData(const CString &serial, Evaluation::CScanResult *result){
	...
	if(MODE_FLUX == result->CheckMeasurementMode()){
		...
		if(scanTime.day == g_selectedDate.day && scanTime.month == g_selectedDate.month && scanTime.year == g_selectedDate.year){
	...
	for(int i = 0; i < result->GetEvaluatedNum(); ++i){
		...
		m_specData[scannerIndex][i-nIgnored].m_peakSaturation  = result->GetPeakIntensity(i);
		m_specData[scannerIndex][i-nIgnored].m_fitSaturation   = result->GetFitIntensity(i);
...
int CEvaluatedDataStorage::AddWindData(const CString &serial, WindSpeedMeasurement::CWindSpeedResult *result){
	...
	if(g_selectedDate.day == result->m_date){
		CWindMeasData wmd;
