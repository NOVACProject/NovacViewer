...
void CGraphCtrl::InvalidateCtrl()
{
	...
	for(double j = m_rectPlot.bottom; j > (m_rectPlot.top - step/2); j -= step){
		...
		m_dcGrid.TextOut (m_rectPlot.left-4, j + 4, number);		
...
void CGraphCtrl::XYPlot(double *xPosition, double *yPosition, double *color, double *xError, double *yError, long pointSum, int plotOption){
	...
	// Get the ranges of the plot area (This must be set after 'PreparePlot' since
	//	'PreparePlot' changes the number of digits in the Y-axis and therefore also the
...
void CGraphCtrl::BarChart2(double *xPosition, double *yValues1, double *yValues2, double *yError1, double *yError2, COLORREF color, int pointSum, int plotOption)
{
	...
	CFont axisFont,*oldFont;
	AxisOptions::FloatRect curAxis; // The current axis (either first or second axis)
	int topLeftx,topLefty,bottomRightx,bottomRighty; // the edges of each bar
	double maxX, minX, maxY, minY;
	double xFactor,yFactor, offsLeft, offsBottom;
	int curYError[2]; // <-- the extreme points for the error-bars in y-direction
	int i, pass;
	...
	for(i = 0; i < pointSum; ++i){
		if(fabs(yValues1[i]) > fabs(yValues2[i])){
			...
			extremeErr[i]	= (yError1 != NULL) ? yError1[i] : 0;
			extremeCol[i]	= m_colors.plot;
			smallV[i]		= yValues2[i];
			smallVErr[i]	= (yError2 != NULL) ? yError2[i] : 0;
			smallCol[i]		= color;
		}else{
			extreme[i]		= yValues2[i];
			extremeErr[i]	= (yError2 != NULL) ? yError2[i] : 0;
			extremeCol[i]	= color;
			smallV[i]		= yValues1[i];
			smallVErr[i]	= (yError1 != NULL) ? yError1[i] : 0;
			smallCol[i]		= m_colors.plot;
		}
	}
	...
	for(pass = 0; pass < 2; ++pass){
		for(i = 0; i < pointSum; ++i){
			...
			// Fill the rectangle
			if(pass == 0)
				m_dcPlot.FillSolidRect(rect, extremeCol[i]);
			else
				m_dcPlot.FillSolidRect(rect, smallCol[i]);

			// Draw the number below the rectangle
			if(plotOption & PLOT_BAR_SHOW_X){
				if(xPosition != NULL){
					strNumber.Format("%.0lf", xPosition[i]);
				}else{
					strNumber.Format("%d", i);
				}
				oldFont = m_dcPlot.SelectObject(&axisFont) ;
				m_dcPlot.TextOut(topLeftx, bottomRighty - 8, strNumber) ;
				m_dcPlot.SelectObject(oldFont);
			}
