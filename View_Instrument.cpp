...
void CView_Instrument::SetLayout(){
	...
	if(this->m_batteryGraph.m_hWnd != NULL){
		m_batteryGraph.MoveWindow(batteryGraphRect);
	}
	// Adjust the width of the column frame
	if(this->m_temperatureGraph.m_hWnd != NULL){
		m_temperatureGraph.MoveWindow(tempGraphRect);
	}
	// Adjust the width of the column frame
	if(this->m_expTimeGraph.m_hWnd != NULL){
		m_expTimeGraph.MoveWindow(expTimeGraphRect);
	}
	// Adjust the width of the column frame
	if(this->m_linkSpeedGraph.m_hWnd != NULL){
		m_linkSpeedGraph.MoveWindow(linkSpeedGraphRect);
	}
}
...
BOOL CView_Instrument::OnInitDialog()
{
	...
	m_toolTip.AddTool(&m_batteryGraph,		IDC_BATTERY_PLOT);
	m_toolTip.AddTool(&m_temperatureGraph,	IDC_TEMPERATURE_PLOT);
	m_toolTip.AddTool(&m_expTimeGraph,		IDC_EXPTIME_PLOT);
	m_toolTip.AddTool(&m_linkSpeedGraph,	IDC_LINKSPEED_PLOT);
...
void CView_Instrument::OnSize(UINT nType, int cx, int cy)
{
