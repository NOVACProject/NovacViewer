...
void CView_Scanner::SetLayout(){
	...
	if(this->m_ColumnFrame.m_hWnd != NULL){
		...
		if(m_columnPlot.m_hWnd != NULL){
			columnPlotRect.top	= 20;
			columnPlotRect.left = 10;
			columnPlotRect.bottom = columnFrameRect.Height() - 10;
			columnPlotRect.right  = columnFrameRect.Width() - 20;
			m_columnPlot.MoveWindow(columnPlotRect);
		}
	}
	...
	if(this->m_fluxFrame.m_hWnd != NULL){
		...
		if(m_fluxPlot.m_hWnd != NULL){
			fluxPlotRect.top	= 20;
			fluxPlotRect.left	= 10;
			fluxPlotRect.bottom = fluxFrameRect.Height() - 10;
			fluxPlotRect.right  = fluxFrameRect.Width() - 20;
			m_fluxPlot.MoveWindow(fluxPlotRect);
		}
